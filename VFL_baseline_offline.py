...
# Anonymization/differential privacy on/off toggle
ANONYMIZE = True
DP = True
DP_CLIP = 1.0
DP_NOISE_MULT = 0.3

ANON_SALT = "blablabla_anonsalt"
def anon_index(df: pd.DataFrame, salt: str) -> pd.DataFrame:
    df = df.copy()
    new_index = []
    for i in df.index.astype(str):
        h = hashlib.sha256((salt + i).encode("utf-8")).hexdigest()
        new_index.append(h)
    df.index = new_index
    return df

def clip_and_noise(t: torch.Tensor, clip_norm: float, noise_multiplier: float) -> torch.Tensor:
    norm = torch.norm(t.detach())
    if norm > clip_norm:
        t = t * (clip_norm / (norm + 1e-12))

    noise = torch.randn_like(t) * (noise_multiplier * clip_norm)
    return t + noise

server_data = pd.read_csv("test-data/outcomeData.csv", delimiter=',', index_col=0)
print(server_data.head())

client1_data = pd.read_csv("test-data/clientoneData.csv", delimiter=',', index_col=0)
client2_data = pd.read_csv("test-data/clienttwoData.csv", delimiter=',', index_col=0)
client3_data = pd.read_csv("test-data/clientthreeData.csv", delimiter=',', index_col=0)
...
if ANONYMIZE == True:
    server_data = anon_index(server_data, ANON_SALT)
    client1_data = anon_index(client1_data, ANON_SALT)
    client2_data = anon_index(client2_data, ANON_SALT)
    client3_data = anon_index(client3_data, ANON_SALT)
...
class VFLClient():
    ...
    def train_model(self):
        ...
        if DP == True:
            self.embedding = clip_and_noise(self.embedding, DP_CLIP, DP_NOISE_MULT)
